...
export function ProjectFilter({ value, onChange, onSearch, onReset }: ProjectFilterProps) {
        ...
        >
          ...
          <div className="relative">
            <select
              ...
              value={value.status}
              onChange={(event) => onChange({ ...value, status: event.target.value as ProjectFiltersValue['status'] })}
            >
          ...
          <button
            className="h-8 rounded border border-sky-700 bg-gradient-to-b from-sky-500 to-sky-700 px-4 text-sm font-semibold text-white hover:from-sky-600 hover:to-sky-800"
            type="submit"
          >
          ...
          <button
            className="h-8 bg-transparent px-2 text-sm text-sky-600 font-medium"
            type="button"
            onClick={onReset}
          >
